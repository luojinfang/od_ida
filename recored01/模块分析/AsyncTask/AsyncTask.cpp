...
AsyncTask::MessageLoop::AddToDelayedWorkQueue


//比较优先级？？
bool __thiscall AsyncTask::MessageLoop::PendingTask::operator<(_DWORD *this, int a2)
...
//================================================================================================================
Address  To       From     Siz Comment               Party 
04BBFD40 51B2390E 51B228FD 20  asynctask.51B228FD    User  //bool __thiscall AsyncTask::MessageLoop::PendingTask::operator<(_DWORD *this, int a2)
04BBFD60 51B2261B 51B2390E 58  asynctask.51B2390E    User	// 排序？
04BBFDB8 51B227B0 51B2261B 38  asynctask.51B2261B    User  //AsyncTask::MessageLoop::AddToDelayedWorkQueue
04BBFDF0 51B24321 51B227B0 2C  asynctask.51B227B0    User
...
char __thiscall AsyncTask::MessageLoop::DoWork(AsyncTask::MessageLoop *this)
{
  ...
  if ( !*((_DWORD *)v1 + 6) ) //判断是否有 task? 
    return 0;
  v2 = (int)v1 + 8;// 51B22776  // 对应  AsyncTask::MessagePumpWin.[8*1]？？ 不同的task队例？
  while ( 1 )
  {
...
void __thiscall AsyncTask::MessageLoop::ReloadWorkQueue(AsyncTask::MessageLoop *this)
{
  ...
  if ( !*((_DWORD *)this + 6) )
  {
	...
	//猜测,这里从全局的task队例里，转移到当前线程处理,所以有锁处理	
	//(_DWORD *)v1 + 29 为全局队例？
    if ( *((_DWORD *)v1 + 33) )
      sub_51B2186B((_DWORD *)v1 + 29, (int)v1 + 8); //v1 和 dowork 使用的 v1 地址可对应
    LeaveCriticalSection((LPCRITICAL_SECTION)((char *)v1 + 136));
  }
}

//把  this  和 a2 交换数据? a2 为 dowork 里,对应 if ( !*((_DWORD *)v1 + 6) ) //判断是否有 task? 
//@a2 为 task 队例？
int __thiscall sub_51B2186B(_DWORD *this, int a2)
{
  ...
  {
	  //51B21878
    v3 = (_DWORD *)*this; //  mov     eax, [edx]   //   edx 和 dowork 使用的 v1 地址可对应,相比后移了两个指针. 比如 当前 edx为 0x19f448,则在dowork里使用的this地址为 0x19f440
    *v2 = *(_DWORD *)a2; //v[0]
    *(_DWORD *)a2 = v3;  
    if ( *v2 )
    {
    ...
    *(_DWORD *)(a2 + 8) = v5;
    ...
    v2[3] = *(_DWORD *)(a2 + 12);
    *(_DWORD *)(a2 + 12) = v6;
    v7 = v2[4]; //v7 单一的数值   //51B218B4 内存断点，看看哪里会修改 v2[4]
    result = *(_DWORD *)(a2 + 16);
    v2[4] = result;
    *(_DWORD *)(a2 + 16) = v7; //拷贝计数?
  }
  return result;
}
